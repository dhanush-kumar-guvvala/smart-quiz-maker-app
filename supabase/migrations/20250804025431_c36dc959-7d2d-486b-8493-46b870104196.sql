-- Drop the username column from quiz_attempts table and all dependent objects
ALTER TABLE quiz_attempts DROP COLUMN IF EXISTS username CASCADE;

-- Drop any remaining triggers and functions
DROP TRIGGER IF EXISTS trigger_set_quiz_attempts_username ON quiz_attempts;
DROP TRIGGER IF EXISTS update_quiz_attempts_username ON quiz_attempts;
DROP FUNCTION IF EXISTS set_quiz_attempts_username() CASCADE;
